...
onegram_data = pd.read_csv('reduced-1-grams.csv', header=None)
le = LabelEncoder()
onegram_data.iloc[:, -1:] = le.fit_transform(onegram_data.iloc[:, -1:])
print(onegram_data.head())
...
model.fit(train_data, train_labels, validation_split=0.2, epochs=50, callbacks=[keras.callbacks.EarlyStopping(patience=5)])
